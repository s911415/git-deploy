...
2. run `sudo chown -R $USER:$SERVER_USER_NAME custom-project-repo-dir/.git/` to change the group of the repo.
...
5. run `sudo -u $SERVER_USER_NAME git pull origin master` to add ssh host key if server user do not connect to git server before.
